...
const VALID_CATEGORIES = new Set(['english', 'math', 'chinese', 'other'])
...
function resolveTaskCategory(taskOrFields, { inferFromName = true } = {}) {
  const category = taskOrFields?.category
  if (VALID_CATEGORIES.has(category)) return category
  if (!inferFromName) return 'other'
  const name = (taskOrFields?.name || '').trim()
  if (TASK_CATEGORY_MAP[name]) return TASK_CATEGORY_MAP[name]
  if (name.includes('数学') || name.includes('口算') || name.includes('学而思')) return 'math'
  if (name.includes('英语') || name.includes('单词')) return 'english'
  if (name.includes('语文') || name.includes('练字') || name.includes('阅读')) return 'chinese'
  return 'other'
}

export function normalizeTaskCategories(data) {
...
export function addTask(data, fields, today, nowMs) {
  ...
  const newTask = {
    ...
    category: resolveTaskCategory(fields, { inferFromName: false }),
    dailyCount: 0,
    lastUpdate: today,
  }
